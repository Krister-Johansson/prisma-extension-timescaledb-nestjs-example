...
import type { CompareResult } from '@/data/aggregates';

export function AggregateCompareChart({ result }: { result: CompareResult }) {
  ...
  return (
    <div className="rounded-[14px] border border-border bg-card p-5 shadow-sm">
      ...
      {data.length === 0 ? (
        <div className="flex h-[200px] items-center justify-center text-center text-[12.5px] text-muted-foreground">
          No hourly data yet — it fills in as the continuous aggregate refreshes.
        </div>
      ) : (
        <>
